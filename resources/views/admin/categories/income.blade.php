    ...
    <div class="box-header clearfix">
      @php
       ...
       foreach($rows as $rowfoto){
        $totalincome = $totalincome +  $rowfoto->total_price;
       }
      @endphp
      <button type="button" class="btn btn-sm btn-danger btn-flat pull-left" style="margin-left: 5px; font-size: 15px;"> Total  : {{ round($totalincome,5)}} LE </button>
    </div>
    <!-- /.box-header -->
...
<div class="row">
    ...
    <div class="col-md-12">
    <form action="{{ route('category.income',$catid) }}" method="GET">
      ...
      <div class="table-responsive">    
        ...
        @php
         $ths = ['Product ( المنتج )' ,'Quantity ( الكمية )','Total Price ( الاجمالي )'];
         $tds = $rows;
         $tdOnly = ['product_id','quantity','total_price'];
         $Otipnsinputs  = [];
        @endphp

        <table class="table table-hover no-margin">
          @include("admin.categories.components.incometableComponent",[$ths,$tds,$tdOnly,$Otipnsinputs])
        </table>
        @else
