        ...
        if($invoice = $row->create($request->toArray()) ){
            ...
            for($i=0;$i <= $request['itrator'];$i++){
                if(isset($request['product_id'.$i])){
                  if( ( $request['product_id'.$i] !="" ) && ( $request['quantity'.$i] !="" ) && ( $request['payprice'.$i] !="" ) && ( $request['total_price'.$i] !="" ) && ( $request['sellprice'.$i] !="" ) && ( $request['total_gain'.$i] !="" ) ){
          ...
          if($request['due'] == 0 ){
            ...
            if( $request['due'] == 0 ) {
                if(isset($request->casherinvoice) && $request->casherinvoice == 1 ) {

                    $pdf = PDF::loadView('Admin.sellInvoice.printCasherSingleInvoice',['row'=>$invoice]);
//                  $pdf->stream('invoice.pdf');
                    swal()->button('Close Me')->message('تم',' تم اضافة الفاتورة بنجاح  ','info');
                }else{
